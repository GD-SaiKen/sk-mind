#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""OA工作流 + 免密登录 + 附件上传下载 - 分模块生成独立Excel"""
import openpyxl
...
def write_overview(ws, modules, title):
    for k,v in OV_C_W.items(): ws.column_dimensions[k].width = v
    ws.merge_cells('A1:E1')
    c = ws['A1']; c.value = title; c.font = Font(name='微软雅黑',size=14,bold=True)
...
def gen_one(path, title, modules):
    wb = openpyxl.Workbook()
    ws = wb.active; ws.title = '接口总览'
    write_overview(ws, modules, title)
    for mod in modules:
        for api in mod['interfaces']:
            sn = api['name']
    ...
    wb.save(path)
    print(f'OK: {path} ({len(wb.sheetnames)} sheets)')
...
'name': 'OA工作流', 'interfaces': [
    {'id': '1', 'name': '查询已送审单据', 'url': 'https://openapi.xiekeyun.com/workflowBills/approvalList.json', 'method': 'POST',
     'desc': '查询需要OA审批的业务单据列表（单据状态=草稿且已送审）',
     'params': [
         ('erpCode', '请求者用户ERP帐号', '必填', 'String'),
         ('billsType', '单据类型（1供应商对账单/2费用单/3发票单/4询价单/5潜在供应商/8检验单/9-ECN报告/10采购单/11核价单/12招标单/13付款申请单/14请购单/16项目招标/17核价失效单/18竞价/19多物料竞价/20准入评估报告/21索样通知单/22供应商信息变更/23供应商合同/15退出通知/24采购方付款单/25内部商城引入单/26预付款单/27供应商通知/28物料停产报告/29品类管理单/30询价比价单/31招标比价单/32图纸变更通知/33采购变更单/34供应计划排程/35询价报价单/36招标报价单/37项目招标投标单/38询价需求单/39关务资料/40收货单/43入库单/44样品管理单/45物料申请单/46委外退料单/47委外申请单/48供应商申诉单/49绩效评估报告/50供应商登记/53工序核价单/55交料通知单/56送货单申请）', '必填', 'Number'),
     ...
     'responses': [
         ...
         (0, 'errorCode', '接口错误编码', 'String'), (0, 'errorMsg', '接口错误信息', 'String'),
         (0, 'dataList', '已送审单据列表', 'Json数组'),
         (1, 'billsType', '单据类型编码', 'Number'), (1, 'innerVendorCode', '供应商编码（仅潜在供应商类型返回）', 'String'),
         (1, 'billXkNo', '平台单据号', 'String'), (1, 'status', '单据状态', 'Number'),
         (1, 'employeeName', '员工姓名', 'String'), (1, 'employeeMobile', '手机号', 'String'),
         (1, 'employeeNumber', '工号', 'String'), (1, 'erpAccount', 'ERP帐号', 'String'),
         (1, 'erpEmployeeCode', 'ERP员工编码', 'String'), (1, 'checkDesc', '送审说明', 'String'),
         (1, 'checkDateTime', '送审时间', 'Date'),
     ]},
    {'id': '2', 'name': 'OA回写审批信息', 'url': 'https://openapi.xiekeyun.com/workflowBills/publish.json', 'method': 'POST',
     'desc': '将OA审批过程及结果回写到携客云平台对应单据，支持直接发布',
     'params': [
         ('erpCode', '请求者用户ERP帐号', '必填', 'String'),
     ...
     'responses': [(0, 'result', '返回结果状态', 'Number'), (0, 'errorCode', '接口错误编码', 'String'), (0, 'errorMsg', '接口错误信息', 'String')]},
    {'id': '3', 'name': '回写审批URL', 'url': 'https://openapi.xiekeyun.com/workflowBills/updateOAViewUrl.json', 'method': 'POST',
     'desc': '回写单据在OA系统中的审批查看URL，方便用户在携客云跳转OA查看进度',
     'params': [('erpCode', '请求者用户ERP帐号', '必填', 'String'), ('billsType', '单据类型', '必填', 'Number'),
                ('billXkNo', '平台单据号', '必填', 'String'), ('oaViewUrl', 'OA系统中的审批查看URL', '必填', 'String')],
     'responses': [(0, 'result', '返回结果状态', 'Number'), (0, 'errorCode', '接口错误编码', 'String'), (0, 'errorMsg', '接口错误信息', 'String')]},
    {'id': '4', 'name': '查询待审批单据列表', 'url': 'https://openapi.xiekeyun.com/workflowBills/getCheckInfoList.json', 'method': 'POST',
     'desc': '查询需要当前人员审批的单据列表',
     'params': [('erpCode', '请求者用户ERP帐号', '必填', 'String'), ('billsType', '单据类型', '必填', 'Number'),
                ('crossEmployeeFlag', '是否跨员工查询（0否/1是）', '选填', 'Number')],
     'responses': [
         (0, 'result', '返回结果状态', 'Number'), (0, 'errorCode', '接口错误编码', 'String'), (0, 'errorMsg', '接口错误信息', 'String'),
         (0, 'dataList', '待审批单据列表', 'Json数组'),
         (1, 'billXkNo', '平台单据号', 'String'), (1, 'billShowNo', '显示单号（ERP单号）', 'String'),
         (1, 'billsType', '单据类型', 'String'), (1, 'wfTypeCode', '流程类型编码', 'String'),
         (1, 'wfnName', '审批关卡名称', 'String'), (1, 'checkUserName', '审批人姓名', 'String'),
         (1, 'checkDesc', '审批说明', 'String'), (1, 'status', '审批状态', 'Number'),
         (1, 'sourceType', '来源类型', 'Number'), (1, 'applyCheckFlag', '是否申请加签', 'Number'),
         (1, 'recallFlag', '是否可撤回', 'Number'), (1, 'createTime', '创建时间', 'Number'),
    ...
    {'id': '5', 'name': '查询审批记录列表', 'url': 'https://openapi.xiekeyun.com/workflowBills/getCheckRecordList.json', 'method': 'POST',
     ...
     'responses': [
         ...
         (1, 'billXkNo', '平台单据号', 'String'), (1, 'billShowNo', '显示单号', 'String'),
         (1, 'billsType', '单据类型', 'String'), (1, 'wfTypeCode', '流程类型编码', 'String'),
         (1, 'wfnName', '审批关卡名称', 'String'), (1, 'checkUserName', '审批人姓名', 'String'),
         (1, 'checkDesc', '审批意见', 'String'), (1, 'status', '审批状态（1通过/2退回/3加签）', 'Number'),
         (1, 'sourceType', '来源类型', 'Number'), (1, 'applyCheckFlag', '是否申请加签', 'Number'),
         (1, 'recallFlag', '是否可撤回', 'Number'), (1, 'checkDateTime', '审批时间', 'Number'),
     ]},
]}
...
'name': '免密登录', 'interfaces': [
    {'id': '1', 'name': '获取免密登录URL', 'url': 'https://openapi.xiekeyun.com/system/ssoLogin.json', 'method': 'POST',
     ...
     'responses': [
         (0, 'result', '返回结果状态 1成功；9失败', 'Number'), (0, 'errorCode', '接口错误编码', 'String'),
         (0, 'errorMsg', '接口错误信息', 'String'), (0, 'data', '免密登录数据', 'Json对象'),
         (1, 'ssoUrl', '免密登录URL（有效期30秒）', 'String'), (1, 'employeeName', '员工姓名', 'String'),
     ]},
]}
...
'name': '附件上传下载', 'interfaces': [
    {'id': '1', 'name': '获取附件下载地址', 'url': 'https://fileapi.xiekeyun.com/download/getUrls.json', 'method': 'POST',
     ...
     'params': [
         ...
         ('downloadUnauditedCert', '是否下载未审核资质', '选填', 'Number'),
     ...
     'responses': [
         ...
         (2, 'fileName', '附件文件名称', 'String'), (2, 'downloadUrl', '附件下载地址（有效期10分钟）', 'String'),
         (2, 'downloadSorceType', '附件来源类型（1采购方/2供应商/3电子签章）', 'Number'),
         (2, 'innerVendorCode', '供应商内部编码（供应商上传时有值）', 'String'),
         (1, 'lineFileList', '单身附件列表', 'Json数组'),
         (2, 'lineNo', '项次号', 'String'), (2, 'billXkNo', '单据平台单号', 'String'),
         (2, 'productCode', '物料编码', 'String'), (2, 'fileName', '附件文件名称', 'String'),
         (2, 'downloadUrl', '附件下载地址', 'String'),
         (1, 'workflowFileList', '审批流附件列表', 'Json数组'),
         (2, 'fileName', '附件文件名称', 'String'), (2, 'downloadUrl', '附件下载地址', 'String'),
         (2, 'workflowFileType', '审批流附件类型（1送审/2审批过程）', 'Number'),
     ]},
    {'id': '2', 'name': '获取附件上传地址', 'url': 'https://fileapi.xiekeyun.com/upload/getUrls.json', 'method': 'POST',
     ...
     'params': [
         ...
         ('autoCreateProdGraph', '是否自动创建图纸版本（0否/1是）', '选填', 'Number'),
         ('validFlag', '是否为最新有效版本（0否/1是）', '选填', 'Number'),
     ],
     'responses': [
         (0, 'result', '返回结果状态', 'Number'), (0, 'errorCode', '接口错误编码', 'String'), (0, 'errorMsg', '接口错误信息', 'String'),
         ...
         (2, 'lineNo', '项次号', 'String'), (2, 'productCode', '物料编码', 'String'),
         (2, 'allowedMaxFileSize', '单身剩余最大上传大小（字节）', 'Number'),
...
BASE = 'G:\\workspace\\GD-SaiiKen\\sk-mind\\docs\\数据梳理\\SRM\\梳理后'
gen_one(f'{BASE}\\6OA工作流_接口梳理.xlsx', 'OA工作流 - API接口总览', [OA])
gen_one(f'{BASE}\\7免密登录_接口梳理.xlsx', '免密登录 - API接口总览', [SL])
gen_one(f'{BASE}\\9附件上传下载_接口梳理.xlsx', '附件上传下载 - API接口总览', [FL])
