<template>
  ...
  <Crud :filter-items="filterItems" v-model:filter-values="filterValues" :pagination="indexPagination">
...
const filterItems: FilterItem[] = [
  ...
  { key: 'system', type: 'select', placeholder: '选择系统', width: '160px',

    options: [

      { label: '全部系统', value: '' },

      { label: 'ERP (E10)', value: 'erp' },

      { label: 'MES (LightMES)', value: 'mes' },
...
async function load() {
  ...
  try {
    ...
    if (filterValues.value.system) params.system = filterValues.value.system
