...
interface SettingItem {
  ...
  color?: string; maskingRule?: string; key?: string; value?: string; usageCount?: number
}
...
// ---- 数据源类型分页 ----
const dsPagination = reactive({ page: 1, pageSize: 20, total: 0, onPageChange() {}, onSizeChange() {} })
...
watch([filteredDataSourceTypes, () => dsPagination.pageSize], () => {
  ...
  if (dsPagination.page > 1 && (dsPagination.page - 1) * dsPagination.pageSize >= dsPagination.total) dsPagination.page = 1
})

// ---- 接入方式分页 ----
const imPagination = reactive({ page: 1, pageSize: 20, total: 0, onPageChange() {}, onSizeChange() {} })
...
watch([filteredIngestionMethods, () => imPagination.pageSize], () => {
  ...
  if (imPagination.page > 1 && (imPagination.page - 1) * imPagination.pageSize >= imPagination.total) imPagination.page = 1
})

// ---- 质量状态分页 ----
const qsPagination = reactive({ page: 1, pageSize: 20, total: 0, onPageChange() {}, onSizeChange() {} })
...
watch([filteredQualityStatuses, () => qsPagination.pageSize], () => {
  ...
  if (qsPagination.page > 1 && (qsPagination.page - 1) * qsPagination.pageSize >= qsPagination.total) qsPagination.page = 1
})

// ---- 敏感字段类型分页 ----
const stPagination = reactive({ page: 1, pageSize: 20, total: 0, onPageChange() {}, onSizeChange() {} })
...
watch([filteredSensitiveTypes, () => stPagination.pageSize], () => {
  ...
  if (stPagination.page > 1 && (stPagination.page - 1) * stPagination.pageSize >= stPagination.total) stPagination.page = 1
})

// ---- 平台参数分页 ----
const ppPagination = reactive({ page: 1, pageSize: 20, total: 0, onPageChange() {}, onSizeChange() {} })
...
watch([filteredPlatformParams, () => ppPagination.pageSize], () => {
  ...
  if (ppPagination.page > 1 && (ppPagination.page - 1) * ppPagination.pageSize >= ppPagination.total) ppPagination.page = 1
})
...
.tag-card { :deep(.el-card__body) { padding: 16px; display: flex; flex-direction: column; gap: 8px; } }
