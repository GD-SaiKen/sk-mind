<template>
  <div class="catalog-page">
    <div class="breadcrumb">
      ...
      <span class="bread-current">数据目录</span>
    </div>
    <div class="tab-bar">
      <button
        v-for="tab in tabs"
    ...
    >
      <el-card
        ...
        :class="[
          'dataset-card',
          ds.quality === 'error' ? 'border-red' : ds.quality === 'warning' ? 'border-yellow' : '',
        ]"
      >
        <div class="ds-top">
        ...
        <div class="ds-meta-grid">
          ...
          <div :class="['ds-meta-item', { 'text-danger': ds.records === 0 }]">
            <el-icon
              :size="14"
              :class="ds.records === 0 ? 'text-danger' : 'text-gray'"
            >
              <Grid />
            </el-icon>
    ...
    <el-empty
      v-else-if="activeTab !== '字段目录'"
      description="没有找到匹配的数据集"
    />
    ...
    >
      ...
      >
        ...
        >
          <template #default="{ row }">
            <el-tag
              :type="row.quality === 'pass' ? 'success' : row.quality === 'warning' ? 'warning' : 'info'"
              effect="plain"
              size="small"
            >
...
const datasets: Dataset[] = [
  ...
  {
    id: 2,
    name: 'mes_production_records',
    displayName: 'MES 生产记录',
    source: 'Plataine MES',
    layer: 'Serving',
    records: 856,
    fields: 18,
  ...
  },
  {
    ...
    quality: 'error',
    agentEnabled: true,
    updatedAt: '2026-07-01 12:00',
  },
  {
    id: 6,
    name: 'mes_oee_data',
    displayName: '设备 OEE 数据',
    source: 'MES MQTT',
    layer: 'Raw',
    records: 5200,
    fields: 10,
...
const filteredDatasets = computed(() =>
  datasets.filter((ds) => {
    const matchSearch =
      ds.displayName.toLowerCase().includes(searchTerm.value.toLowerCase()) ||
