<template>
  <div class="page-layout">
      ...
      <el-col :span="6">
        <el-card shadow="never" class="info-card">
          ...
          <div class="info-card-foot">
            <span>健康率</span>
            <div class="health-bar"><div class="health-bar-fill" :style="{ width: healthRate + '%' }" /></div>
    ...
    <div v-if="filteredDatasets.length > 0 && activeTab !== '字段目录'" class="dataset-grid">
      <el-card v-for="ds in filteredDatasets" :key="ds.id" shadow="never" :class="['dataset-card', ds.quality === 'error' ? 'border-red' : ds.quality === 'warning' ? 'border-yellow' : '']">
        <div class="ds-top">
          <div class="ds-name-row">
            ...
            <el-tag effect="plain" :type="ds.quality === 'error' ? 'danger' : ds.quality === 'warning' ? 'warning' : 'success'">
              {{ ds.quality === 'error' ? '异常' : ds.quality === 'warning' ? '警告' : '正常' }}
            </el-tag>
          </div>
        ...
        <div class="ds-meta-grid">
          <div class="ds-meta-item"><el-icon :size="14" class="text-gray"><OfficeBuilding /></el-icon><span>{{ ds.source }}</span></div>
          <div class="ds-meta-item"><el-icon :size="14" class="text-gray"><Clock /></el-icon><span>{{ ds.updatedAt.split(' ')[0] }}</span></div>
          ...
          <div class="ds-meta-item"><el-icon :size="14" class="text-gray"><Collection /></el-icon><span>{{ ds.fields }} 个字段</span></div>
        </div>
        <div class="ds-actions">
          <el-button v-if="ds.agentEnabled && ds.records > 0" plain type="primary"><el-icon :size="14"><Service /></el-icon>问问 Agent</el-button>
          <el-button v-else-if="ds.agentEnabled" plain disabled><el-icon :size="14"><Service /></el-icon>暂无可查数据</el-button>
          <el-button v-else plain disabled><el-icon :size="14"><Lock /></el-icon>申请权限</el-button>
          <router-link :to="'/tables/'"><el-button link type="primary">{{ ds.records === 0 ? '查看表结构' : '查看详情' }}</el-button></router-link>
        </div>
      </el-card>
    </div>

    <el-empty v-else-if="activeTab !== '字段目录'" description="没有找到匹配的数据集" />

    <div v-if="activeTab === '字段目录'" class="field-catalog">
      <el-table :data="fieldCatalogData" stripe>
        <el-table-column prop="fieldName" label="字段名" width="160" />
        <el-table-column prop="displayName" label="显示名" width="130" />
        <el-table-column prop="dataset" label="所属数据集" width="160" />
        <el-table-column prop="meaning" label="字段含义" min-width="160" />
        ...
        <el-table-column label="质量" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="row.quality === 'pass' ? 'success' : row.quality === 'warning' ? 'warning' : 'info'" effect="plain" size="small">
              {{ row.quality === 'pass' ? '正常' : row.quality === 'warning' ? '警告' : '—' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="语义映射" width="160">
          <template #default="{ row }">
            <el-tag v-if="row.semanticMapping" effect="plain" size="small">{{ row.semanticMapping }}</el-tag>
            <span v-else>—</span>
          </template>
        </el-table-column>
      </el-table>
...
const fieldCatalogData = [
  ...
  { fieldName: 'customer_name', displayName: '客户名称', dataset: '客户信息表', meaning: '客户企业全称', type: 'VARCHAR(128)', sensitive: false, sensitiveType: '', quality: 'pass', semanticMapping: '客户.客户名称' },
  { fieldName: 'amount', displayName: '订单金额', dataset: '销售订单表', meaning: '订单总金额(元)', type: 'DECIMAL(12,2)', sensitive: false, sensitiveType: '', quality: 'pass', semanticMapping: '订单.订单金额' },
...
.info-card-icon { display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 8px; &.bg-blue { background: #dbeafe; color: $color-primary; } &.bg-green { background: #dcfce7; color: $color-success; } &.bg-yellow { background: #fef3c7; color: $color-warning; } &.bg-purple { background: #ede9fe; color: #7c3aed; } }
.info-card-label { font-size: $font-size-base; color: $color-text-secondary; }
.info-card-subtag { font-size: $font-size-xs; color: $color-text-placeholder; background: #f3f4f6; padding: 1px 6px; border-radius: 4px; margin-left: auto; white-space: nowrap; }
.info-card-value-row { display: flex; align-items: baseline; gap: 8px; }
.info-card-value { font-size: 28px; font-weight: $font-weight-bold; color: $color-text-primary; &.green { color: $color-success; } &.yellow { color: $color-warning; } }
.badge { font-size: $font-size-xs; padding: 1px 6px; border-radius: 4px; &.blue { color: $color-primary; background: #eff6ff; } &.neutral { color: $color-text-placeholder; background: #f3f4f6; } }
...
.health-bar-fill { height: 100%; background: $color-success; border-radius: 2px; transition: width 0.3s; }
...
.ds-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; &.green { background: $color-success; } &.yellow { background: $color-warning; } &.red { background: $color-danger; } }
.ds-name { font-size: $font-size-base; font-weight: $font-weight-semibold; color: $color-text-primary; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.ds-meta-item { display: flex; align-items: center; gap: 4px; font-size: $font-size-xs; color: $color-text-secondary; }
.text-gray { color: $color-text-placeholder; }
.text-danger { color: $color-danger; }
.ds-actions { display: flex; align-items: center; justify-content: space-between; padding-top: 12px; border-top: 1px solid $color-border-light; }
</style>
