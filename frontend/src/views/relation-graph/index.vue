    ...
    <el-row :gutter="16" class="stat-row">
      ...
      <el-col :span="6">
        <el-card shadow="never" class="info-card">
          ...
          <div class="foot"><span>确认率</span><div class="health-bar"><div class="health-bar-fill" :style="{ width: confirmRate + '%' }" /></div><span>{{ confirmRate }}%</span></div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never" class="info-card">
      ...
      <el-col :span="6">
        <el-card shadow="never" class="info-card">
          ...
          <div class="val-row"><span class="val yellow">{{ pendingCount }}</span><span class="badge neutral">较昨日持平</span></div>
          <div class="foot">{{ pendingCount > 0 ? '请及时确认' : '暂无待确认' }}</div>
        </el-card>
      </el-col>
    </el-row>

    <Crud v-if="activeTab === '关系边列表'" :filter-items="searchFilterItems" v-model:filter-values="searchValues" :pagination="edgesPagination">
    ...
    <el-card v-if="activeTab === '关系查询'" shadow="never">
      <div class="query-form">
        <el-row :gutter="16">
          <el-col :span="8">
            <label>对象类型</label>
            <el-select v-model="queryType" class="w-full">
          ...
          <el-col :span="8">
            <label>对象ID</label>
            <el-input v-model="queryId" placeholder="如: CUST-8856" />
          </el-col>
          <el-col :span="8">
            <label>跳数范围</label>
...
const edgesColumns: ColumnSchema[] = [
  ...
  { type: 'tag', prop: 'relation', label: '关系', width: 120, tagType: 'warning' },
  { type: 'custom', prop: 'toEntity', label: '目标实体', minWidth: 160 },
  { type: 'text', prop: 'source', label: '来源', width: 140 },
  { type: 'tag', prop: 'generatedBy', label: '生成方式', width: 100, tagMap: { 'AI生成': 'warning' } },
