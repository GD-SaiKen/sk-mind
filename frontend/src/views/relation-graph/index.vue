<template>
  <div class="rg-page">
    <div class="breadcrumb">
      ...
      <span class="bread-current">关系图谱</span>
    </div>
    <div class="tab-bar">
      <button
        v-for="tab in tabs"
    ...
    >
      <el-col :span="6">
        ...
        >
          ...
          <div class="val-row">
            <span class="val">{{ relationEdges.length }}</span>
            <span class="badge green-bg">↑ 2 较昨日</span>
          </div>
      ...
      <el-col :span="6">
        ...
        >
          ...
          <div class="val-row">
            <span class="val green">{{ confirmedCount }}</span>
      ...
      <el-col :span="6">
        ...
        >
          <div class="info-card-header">
            <div class="info-card-icon bg-purple">
      ...
      <el-col :span="6">
        ...
        >
          ...
          <div class="val-row">
            <span class="val yellow">{{ pendingCount }}</span>
            <span class="badge neutral">较昨日持平</span>
          </div>
          <div class="foot">{{ pendingCount > 0 ? '请及时确认' : '暂无待确认' }}</div>
        </el-card>
      </el-col>
    </el-row>

    <Crud
      v-if="activeTab === '关系边列表'"
      :filter-items="searchFilterItems"
      v-model:filter-values="searchValues"
      :pagination="edgesPagination"
    >
      <template #actions>
        <el-button
          type="primary"
    ...
    <div
      v-if="activeTab === '待确认关系'"
      class="pending-list"
    >
      <div
        v-for="edge in pendingEdges"
        :key="edge.id"
        class="pending-card"
      >
        <div class="pending-header">
          <div class="pending-path">
            <el-tag effect="plain">{{ edge.fromEntity }}</el-tag>
            <span class="mono">{{ edge.fromId }}</span>
            <span class="arrow">→ {{ edge.relation }} →</span>
            <el-tag effect="plain">{{ edge.toEntity }}</el-tag>
        ...
        <div class="pending-meta">
          来源: {{ edge.source }} · 生成方式: {{ edge.generatedBy }} · 可信度: {{ (edge.confidence * 100).toFixed(0) }}%
        </div>
...
import {
  Search, Plus, Edit, Connection, CircleCheckFilled, Link, WarningFilled,
...
const filteredEdges = computed(() =>
  relationEdges.filter(e => {
    ...
    return !s ||
      e.fromEntity.includes(s) ||
      e.toEntity.includes(s) ||
      e.relation.includes(s) ||
      e.source.includes(s)
  }),
)
...
const pagedEdges = computed(() =>
  slicePage(filteredEdges.value, edgesPagination.page, edgesPagination.pageSize),
)
watch([filteredEdges, () => edgesPagination.pageSize], () => {
  edgesPagination.total = filteredEdges.value.length
})
...
const confirmRate = computed(() =>
  relationEdges.length > 0
    ? Math.round(confirmedCount.value / relationEdges.length * 100)
    : 0,
)
