    ...
    <div class="stat-grid">
      <div v-for="card in statCards" :key="card.label" class="stat-card">
        <div class="sc-top">
          ...
          <el-tag v-if="card.badge" size="small" :type="card.badgeType" effect="plain">{{ card.badge }}</el-tag>
        </div>
              ...
              <div class="inline-progress">
                ...
                <div class="ip-row">
                  <el-progress
                    ...
                    style="width: 100px"
                  />
                  <el-button size="small" type="danger" text @click="handleCancel(row.id, running[row.id].batchId)">停止</el-button>
                </div>
              </div>
            </template>
            <template v-else>
              <div class="action-btns">
                <el-button size="small" @click="router.push(`/ingestion/${row.id}`)">详情</el-button>
                <el-button size="small" type="success" :icon="VideoPlay" @click="handleExecute(row)">执行</el-button>
                <el-button v-if="row.status !== 'disabled'" size="small" text type="danger" @click="handleDelete(row)">停用</el-button>
              </div>
            </template>
          </template>
        </Table>
