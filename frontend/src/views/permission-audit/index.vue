<template>
  <div class="perm-page">
    <div class="breadcrumb">
      ...
      <span class="bread-current">权限审计</span>
    </div>
    <div class="tab-bar">
      <button
        v-for="tab in tabs"
      ...
      <el-col :span="6">
        ...
        >
          ...
          <div class="val-row">
            ...
            <span class="badge green-bg">↑ 3 较昨日</span>
          </div>
    ...
    >
      ...
      <template #table>
        <Table
          :columns="sfColumns"
          :data="pagedSensitiveFields"
        />
      </template>
    </Crud>

    <el-card
      v-if="activeTab === '用户管理' || activeTab === '数据集权限'"
      shadow="never"
    >
...
import {
  Search, Plus, View, User, Lock, CircleCheckFilled, WarningFilled, Edit, Coin, Upload, Service,
...
const filteredAuditLogs = computed(() =>
  auditLogs.filter(
    l =>
      l.user.includes(searchValues.value.keyword || '') ||
      l.object.includes(searchValues.value.keyword || '') ||
      l.operation.includes(searchValues.value.keyword || ''),
  ),
)
const filteredSensitiveFields = computed(() =>
  sensitiveFields.filter(
    f =>
      f.field.includes(searchValues.value.keyword || '') ||
      f.dataset.includes(searchValues.value.keyword || ''),
  ),
)
...
const pagedAuditLogs = computed(() =>
  slicePage(filteredAuditLogs.value, auditPagination.page, auditPagination.pageSize),
)
watch([filteredAuditLogs, () => auditPagination.pageSize], () => {
  auditPagination.total = filteredAuditLogs.value.length
})

const sfPagination = reactive({
  page: 1,
  pageSize: 20,
  total: 0,
  onPageChange() {},
...
const pagedSensitiveFields = computed(() =>
  slicePage(filteredSensitiveFields.value, sfPagination.page, sfPagination.pageSize),
)
