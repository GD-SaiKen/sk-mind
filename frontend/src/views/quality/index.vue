...
const rulesFilterItems: FilterItem[] = [
  ...
  { key: 'type', type: 'select', placeholder: '规则类型', width: '120px', options: [{ label: '全部类型', value: '' }, { label: '完整性', value: '完整性' }, { label: '唯一性', value: '唯一性' }, { label: '格式', value: '格式' }] },
]
...
const rulesColumns: ColumnSchema[] = [
  ...
  { type: 'tag', prop: 'status', label: '状态', width: 90, tagMap: { success: 'success', warning: 'warning', error: 'danger' }, formatter: (v: string) => ({ success: '通过', warning: '警告', error: '异常' }[v] ?? v) },
  { type: 'text', prop: 'lastRun', label: '最近执行', width: 180 },
  { type: 'action', label: '操作', width: 120, buttons: [{ label: '执行', icon: VideoPlay, onClick: () => {} }, { label: '编辑', icon: Edit, onClick: () => {} }] },
]
...
const recordsColumns: ColumnSchema[] = [
  ...
  { type: 'tag', prop: 'result', label: '结果', width: 100, tagMap: { '通过': 'success', '发现问题': 'warning' } },
...
.info-card-icon { display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 8px; &.bg-blue { background: #dbeafe; color: $color-primary; } &.bg-green { background: #dcfce7; color: $color-success; } &.bg-yellow { background: #fef3c7; color: $color-warning; } &.bg-red { background: #fee2e2; color: $color-danger; } }
...
.info-card-value { font-size: 28px; font-weight: $font-weight-bold; color: $color-text-primary; &.green { color: $color-success; } &.yellow { color: $color-warning; } &.red { color: $color-danger; } }
...
.issue-card { padding: 16px; background: #fefce8; border: 1px solid #fef08a; border-radius: $radius-base; }
