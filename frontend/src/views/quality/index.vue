<template>
  <div class="quality-page">
    <div class="breadcrumb">
      ...
      <span class="bread-current">数据质量</span>
    </div>
    <div class="tab-bar">
      <button
        v-for="tab in tabs"
      ...
      <el-col :span="6">
        ...
        >
          <div class="info-card-header">
            ...
            <span class="info-card-subtag danger-tag">
              {{ issueCount > 0 ? '需处理' : '正常' }}
            </span>
          </div>
          <div class="info-card-value-row">
    ...
    <Crud
      ...
      :filter-items="recordsFilterItems"
      v-model:filter-values="filterValues"
      ...
      >
        ...
        <div class="issue-meta-row">
          <div class="issue-meta-item">
            <span class="meta-label">问题数量</span>
            <span class="meta-value">{{ issue.count }} 条</span>
          </div>
          <div class="issue-meta-item">
            <span class="meta-label">样例值</span>
            <span class="meta-value mono">{{ issue.sample }}</span>
          </div>
          <div class="issue-meta-item">
            <span class="meta-label">影响范围</span>
            <span class="meta-value">{{ issue.impact }}</span>
          </div>
          <div class="issue-meta-item">
            <span class="meta-label">负责人</span>
            <span class="meta-value">{{ issue.owner }}</span>
          </div>
        </div>
...
const filteredRules = computed(() =>
  qualityRules.filter(r => {
    const matchSearch =
      r.name.includes(filterValues.value.keyword || '') ||
      r.dataset.includes(filterValues.value.keyword || '')
...
const filteredRecords = computed(() =>
  executionRecords.filter(r =>
    r.rule.includes(filterValues.value.keyword || '') ||
    r.dataset.includes(filterValues.value.keyword || ''),
  ),
)
...
const pagedRules = computed(() =>
  slicePage(filteredRules.value, rulesPagination.page, rulesPagination.pageSize),
)
watch([filteredRules, () => rulesPagination.pageSize], () => {
  rulesPagination.total = filteredRules.value.length
  if (
    rulesPagination.page > 1 &&
    (rulesPagination.page - 1) * rulesPagination.pageSize >= rulesPagination.total
  ) {
    rulesPagination.page = 1
  }
})
...
const pagedRecords = computed(() =>
  slicePage(filteredRecords.value, recordsPagination.page, recordsPagination.pageSize),
)
watch([filteredRecords, () => recordsPagination.pageSize], () => {
  ...
  if (
    ...
    (recordsPagination.page - 1) * recordsPagination.pageSize >= recordsPagination.total
  ) {
    recordsPagination.page = 1
  }
})
