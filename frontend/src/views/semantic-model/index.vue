<template>
  <div class="sem-page">
    <div class="breadcrumb">
      ...
      <span class="bread-current">语义模型</span>
    </div>
    <div class="tab-bar">
      <button
        v-for="tab in tabs"
      ...
      <el-col :span="6">
        ...
        >
          <div class="info-card-header">
            <div class="info-card-icon bg-indigo">
...
import {
  Search, Plus, Edit, Service, Collection, Connection, Link, Select,
...
function filterBySearch<T extends Record<string, any>>(items: T[], fields: string[]): T[] {
  ...
  return items.filter(item =>
    fields.some(f => String(item[f] ?? '').includes(searchValues.value.keyword || '')),
  )
}
...
watch([filteredObjects, () => soPagination.pageSize], () => {
  soPagination.total = filteredObjects.value.length
})
...
watch([filteredRelations, () => srPagination.pageSize], () => {
  srPagination.total = filteredRelations.value.length
})
...
watch([filteredMappings, () => dmPagination.pageSize], () => {
  dmPagination.total = filteredMappings.value.length
})
...
watch([filteredPolicies, () => apPagination.pageSize], () => {
  apPagination.total = filteredPolicies.value.length
})
