...
| 3 | BUDGET_ID | 主键 | GUID | Y | Y |
| 4 | BUDGET_GROUP_ID | 预算组 | GUID | Y |  |
| 5 | BUDGET_ITEM_ID | 预算项 | GUID | Y |  |
| 6 | ADMIN_UNIT_ID | 部门 | GUID | Y |  |
| 7 | ACCOUNT_YEAR | 会计年度 | string(8) | Y |  |
| 8 | BUDGET_PERIODS | 预算期数 | number | Y |  |
| 9 | CONTINUE_FLAG | 后置单据跨年延用执行标识 | number(0/1) | Y |  |
| 10 | CONTROL_STRATEGY | 控制策略 | number | Y |  |
| 11 | ANNUAL_BUDGET_AMT | 年度预算金额 | number(23,8) | Y |  |
