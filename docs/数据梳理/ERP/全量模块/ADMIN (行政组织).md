...
- **ADMIN_UNIT (行政单元基本信息)**
- **ADMIN_UNIT_CSTELEMENT (部门成本要素)**
- **ADMIN_UNIT_CSTELEMENT_D**
- **ADMIN_UNIT_D (行政单元属性明细)**
- **ADMIN_UNIT_LOG**
...
| 3 | ADMIN_UNIT_ID | 主键 | GUID | Y | Y |
| 4 | ADMIN_UNIT_CODE | 行政单元编码 | string(20) | Y |  |
| 5 | ADMIN_UNIT_NAME | 行政单元名称 | string(40) | Y |  |
| 6 | ADMIN_UNIT_LEVEL_CODE | 行政单元阶层编码 | string(80) | Y |  |
| 7 | START_DATE | 生效日期 | date | Y |  |
| 8 | END_DATE | 失效日期 | date | Y |  |
| 9 | VIRTUAL_UNIT | 虚拟组织 | number(0/1) | Y |  |
| 10 | REMARK | 备注 | string(510) | Y |  |
| 11 | SUPER_ADMIN_UNIT_ID | 上级行政单元 | GUID | Y |  |
| 12 | COMPANY_ID | 公司编号 | GUID | Y |  |
| 13 | ADMIN_UNIT_SUPERVISOR_ID | 行政单元主管 | GUID | Y |  |
| 14 | ADMIN_UNIT_EXPRESS_ID | 行政单元收发人 | GUID | Y |  |
| 15 | HISTORICAL | 历史部门标识 | number(0/1) | Y |  |
| 16 | DECISION_LEVEL_ID | 核决层级 | GUID | Y |  |
| 17 | SOURCE | 来源 | string(40) | Y |  |
| 18 | HRM_DATAKEY | HRM传输批次号 | string(80) | Y |  |
...
| 1 | Owner_Dept | 关联部门 | GUID | Y |  |
| 2 | Owner_Emp | 关联员工 | GUID | Y |  |
| 3 | ADMIN_UNIT_CSTELEMENT_ID | 主键 | GUID | Y | Y |
...
| 2 | ADMIN_UNIT_CSTELEMENT_D_ID | 主键 | GUID | Y | Y |
| 3 | ALLOCATION_MODE | 分配方式 | number | Y |  |
| 4 | STD_ALLOC_RATE | 标准分配率 | number(23,8) | Y |  |
| 5 | AMOUNT_FORMULA | 成本取数公式 | string | Y |  |
| 6 | REMARK | 备注 | string(510) | Y |  |
| 7 | COST_ELEMENT_ID | 成本要素 | GUID | Y |  |
| 8 | COST_ALLOC_BASIS_ID | 成本分配依据 | GUID | Y |  |
...
| 33 | Version | 版本号，不要随意更改 | binary | Y |  |
| 34 | ADMIN_UNIT_CSTELEMENT_ID |  | GUID | Y |  |
| 35 | STD_COST_ALLOC_BASIS_VALUE | 标准成本分配依据数值 | number(16,6) | Y |  |

### ADMIN_UNIT_D (行政单元属性明细)

| # | Column | Description | Type | Required | PK |
|---|--------|-------------|------|----------|----|
| 1 | ADMIN_UNIT_D_ID | 主键 | GUID | Y | Y |
| 2 | REMARK | 备注 | string(510) | Y |  |
| 3 | AU_CHARACTER_ID | 属性项编码 | GUID | Y |  |
| 4 | AU_CHARACTER_D_ID | 行政单元属性值 | GUID | Y |  |
...
| 1 | Owner_Dept | 关联部门 | GUID | Y |  |
| 2 | Owner_Emp | 关联员工 | GUID | Y |  |
| 3 | ADMIN_UNIT_LOG_ID | 主键 | GUID | Y | Y |
| 4 | MODIFY_DATE | 修改时间 | date | Y |  |
| 5 | ADMIN_UNIT_ID | 行政单元 | GUID | Y |  |
| 6 | ADMIN_UNIT_SUPERVISOR_ID | 行政单元主管编码 | GUID | Y |  |
| 7 | ADMIN_UNIT_EXPRESS_ID | 行政单元收发人编码 | GUID | Y |  |
